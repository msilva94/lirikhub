...
@Component({
  selector: 'app-battlestationedit',
  templateUrl: './battlestationedit.component.html',
  styleUrls: ['./battlestationedit.component.css']
})
export class BattlestationeditComponent implements OnInit {
  number:Number;
  date:String;
  vods:Vod[];
  private sub:any;
  ...
  ngOnInit() {
    this.titleService.setTitle('Edit Battlestations Review - LIRIK Hub');
  ...
  updateBattlestation() {
    ...
    this.battlestationsService.updateBattlestation(this.number, battlestation).subscribe(data => {
      if(data.success){
        this.router.navigate(['/battlestations']);
      } else {
